...
//global arrays
var colors = new Array, points = new Array, names = new Array, pointsNames = new Array
//whole svg path
...
//players array
//["jméno", body, "barva kšiltovky", "barva batohu", "barva oblečení"] !!! body a barvy psát bez "" !!!
var players = [["Ema", 1, red, lightRed, blue], ["Terka", 1, green, lightGreen, brown], ["Tomáš", 36, green, lightGreen, brown], ["Kika", 50, green, lightGreen, brown], ["Lukáš", 37, green, lightGreen, brown], ["Matěj", 38, green, lightGreen, brown], ["Jakub", 39, green, lightGreen, brown], ["Michal", 15, green, lightGreen, brown], ["Anna", 48, green, lightGreen, brown], ["Víťa", 45, green, lightGreen, brown], ["Kryštof", 12, green, lightGreen, brown], ["Bea", 20, green, lightGreen, brown], ["Aďa", 34, green, lightGreen, brown]]

//get score from array into another single array
for (var p in players) {
  names[p] = players[p][0]
  points[p] = players[p][1]
  pointsNames += names[p]
...
scoreboard2.textContent = points[1] + " " + names[0]
scoreboard3.textContent = points[2] + " " + names[0]
scoreboard4.textContent = points[3] + " " + names[0]
scoreboard5.textContent = points[4] + " " + names[0]
...
sameScore = find_duplicate_in_array(points)

for (var p in players) {
  if(players[p][1].toString === sameScore)
  players[p].push("sameScore")
}

console.log(find_duplicate_in_array(points))
...
for(let set = 0 ; set <= players.length; set++) {
  ...
  playerText[set].setAttribute("x", "50px")

  //center nickaname
  if (players[set][0].length > 10) {
    playerText[set].setAttribute("x", "25px")
  }
  //move nickname to prevent overflowing page
  //from over the person to left
  if (players[set][1] >= 30) {
    playerText[set].setAttribute("x", "-25px")
    playerText[set].setAttribute("y", "5px")
  }
